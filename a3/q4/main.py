...
for _ in tqdm(range(num_steps), desc='Generating trajectory'):
    ...
    visited_states.add(
        tuple(np.concatenate((
            np.floor(R[-1] / grid_size).astype(int),
            np.floor(P[-1] / grid_size).astype(int)
        )).flatten())
    )
    num_acc_states.append(len(visited_states))
...
plt.plot(num_acc_states)
plt.title('Variation in Number of Accessible Microstates with Time')
plt.ylabel('Number of Accessible Microstates')
plt.xlabel('Timestep')
plt.grid(linewidth=0.35)
plt.savefig('num-accessible-microstates.png', bbox_inches='tight')
plt.close()
plt.clf()
